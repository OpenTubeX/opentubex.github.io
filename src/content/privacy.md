...
By default, subscriptions, playlists, settings including saved channel settings, history, watch statistics, profiles, and open tabs remain on your device. Enabling synchronization sends copies of the selected categories to the configured sync server. With enhanced-privacy sync, watch statistics are included by default; turn off **Sync statistics** to exclude them:
